...
5. **竖条空隙补点**（默认开启）：当某个 X 列在另一视图缺覆盖时，仅回退到边界深度/高度补单列条纹，不铺整面
6. **边界四面包围补点**（默认开启）：可选在 `y_min/y_max/z_min/z_max` 四面额外铺面，进一步消除大面积缺口
7. **侧视 Z 向填隙**（默认 2 px）：在侧视掩膜上闭合过小的 Z 向断档
8. **稀疏均匀化**：在保持两向投影的前提下减少点数
...
5. 点击 **导出 STL** 或 **导出 OBJ** 可将当前点阵导出为 `.stl/.obj`（自动剔除内部面，仅保留外表面）
6. 右侧 **投影校验** 面板可对比重建投影与输入是否一致
...
| edge_strip_fill | bool | 仅补空隙竖条，默认 true |

...
2. **深度双面补全**（默认开启）：侧视某列在 Z 向不连续时，不在整段深度填实，而是在该列 **最前、最后** 两个深度面各落点，减轻正视竖条空缺，同时尽量保留侧视中间的空隙
3. **侧视 Z 向填隙**（默认 2 px）：在侧视掩膜上闭合过小的 Z 向断档
4. **稀疏均匀化**：在保持两向投影的前提下减少点数
